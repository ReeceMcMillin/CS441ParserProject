      ...
      (let ([head (first chars)]
        ...
        (match head
          ...
          [#\: (if (eq? (first tail) #\=)
                   ...
                   (error "Error: ':' must be followed by '='"))]
...
;(define-values (tokens tail) (tokenize (string->list "123 abc ")))
(define input-str "read thenumber123 := 123\nwrite abc + 32")
(define chars (string->list input-str))
(define tokens (tokenize chars))
